...
def main():
    ...
    # tmp debugging
    model_output_data = model_output_data.iloc[:1000, :]
    ...
    for model_1, model_2 in model_combos:
        ...
        for _ in range(bootstrap_iters):
            bootstrap_idx_i = np.random.choice(list(range(redundancy_data_1.shape[0])), bootstrap_sample_size, replace=False)
        ...
        redundancy_diff = np.median(redundancy_diff)
        ...
        diversity_diffs = []
        for _ in range(bootstrap_iters):
            bootstrap_idx_i = np.random.choice(list(range(model_output_data.shape[0])), bootstrap_sample_size, replace=False)
        ...
        diversity_diff = np.median(diversity_diffs)
